          ...
          if (data2.length > 0) {
            ...
            bcrypt.genSalt(10, function (err, salt) {
              bcrypt.hash(password, salt, function (err, hash) {
                con.query(
                  "INSERT INTO `users` (`username`, `email`, `firstname`, `lastname`, `password`, `created_at`, `updated_at`) VALUES(?,?,?,?,?,now(), now() )",
  ...
  if ((username != "") | (password != "")) {
    ...
    con.query(
      ...
      function (err, data, fields) {
        ...
        if (data.length > 0) {
          con.query(
            `UPDATE users SET last_login = now() WHERE id = '${req.session.cache[0].id}'`
          );
